...
const querySnapshots = await getDocs(collection(db, "posts"),where("uid","==",uid));
    ...
    <p class="text-[24px] font-bold leading-[1.5] text-[#000] w-[%]">${item.title}</p>
            ...
            del.forEach((btn, index)=> {
                btn.addEventListener('click', async () => {
                    console.log("Delete Called",allBlogsArr[index].userArr.uid);
                    const uid = JSON.stringify(allBlogsArr[index].uid);
